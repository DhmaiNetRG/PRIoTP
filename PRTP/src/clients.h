...
struct client_node {
  ...
  /* PRIoTPS: per-client security state */
  uint8_t  peer_pub_key[32];   /* client's X25519 public key (from SUBSCRIBE HSK) */
  uint32_t session_id;         /* active session ID (0 = no session) */
  uint8_t  handshake_done;     /* 1 = handshake completed */
  struct timeval handshake_sent_at;  /* when subscribe was first sent */
  int            handshake_retries;  /* retry counter (max 3) */
  uint8_t        last_sct[2048];
  size_t         last_sct_len;
};
